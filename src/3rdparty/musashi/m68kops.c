...
unsigned char m68ki_cycles[0x10000]; /* Cycles used by each opcode */
...
typedef struct
{
	...
	unsigned char cycles;      /* cycles the 68000 takes */
	...
	while(ostruct->mask == 0xf1f8)
	{
		for(i = 0;i < 8;i++)
		{
			for(j = 0;j < 8;j++)
			{
				...
				{
					// On the 68000 shift distance affects execution time.
